                ...
                hook -> {
                    ...
                    try {
                        if (Event.eventExists(shortName)) {
                            ...
                            //TODO Fix scheduled event deletion
                            // Event.scheduleEventDeletion(eventDateTime, commandEvent, success);
                            
                        } else {
                            hook.editOriginal("That event type does not exist").queue();
                        }
                    } catch (RaidBotRuntimeException e) {
                        hook.editOriginal("Error occurred while creating event.").queue();
                    }
                }
        );
    }
